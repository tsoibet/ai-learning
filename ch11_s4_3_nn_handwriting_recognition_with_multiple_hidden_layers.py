...
class NeuralNetwork(nn.Module):
  def __init__(self): 
    ...
    self.layers = nn.Sequential( 
      nn.Flatten(),
      nn.Linear(784, 1024), # hidden layer 1
      nn.ReLU(),
      nn.Linear(1024, 1024), # hidden layer 2
      nn.ReLU(),
      nn.Linear(1024, 10)
    )
  def forward(self, x): 
...
torch.save(model.state_dict(), 'sample11_4_3.pth')
torch.save({'losses': losses, 'accuracies': accuracies}, 'sample11_4_3.hst')
print("Done!")

#Visualisation of learning process
import matplotlib.pyplot as plt
history = torch.load('./sample11_4_3.hst')
